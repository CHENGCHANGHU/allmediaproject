...
CREATE TABLE `sys_log` (
  `id` bigint(40) unsigned NOT NULL AUTO_INCREMENT,		/*主键*/
  `ip` varchar(20) NOT NULL,					/*操作地址ip*/
  `createTime` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00' ON UPDATE CURRENT_TIMESTAMP,	/*操作时间*/
  `remark` varchar(255) NOT NULL,				/*操作内容*/
  `operateUrl` varchar(50) NOT NULL,			/*操作的访问地址*/
  `operateBy` varchar(255) NOT NULL,			/*操作的浏览器*/
  PRIMARY KEY (`id`)
...
CREATE TABLE `tbl_user` (
  ...
  `create_by`  TIMESTAMP,							/*操作时间*/
  `modified_by` datetime DEFAULT NULL,				/*修改时间*/
  `idDeleted` tinyint(1) DEFAULT '0',				/*删除标志位*/
  `userStatus` varchar(255) NOT NULL,				/*用户身份标志位 普通用户0；自媒体用户1；管理员用户2*/
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
...
CREATE TABLE `tbl_user_info` (
  ...
  `head_portrait` longblob,						/*头像url*/
  `description` varchar(255) DEFAULT NULL,		/*账号描述*/
  `create_by` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,			/*操作时间*/
  `modified_by` datetime DEFAULT NULL,			/*修改时间*/
  `idDeleted` tinyint(1) DEFAULT '0',			/*删除标志位*/
  PRIMARY KEY (`id`)
